...
bool UVdjmRecordEnvResolver::ResolveEnvPlatform(const FVdjmRecordEnvPlatformPreset* presetData)
{
	...
	const int32 MinFrameRate = FMath::Max(1, GlobalRules.MinFrameRate);
	const int32 MaxFrameRate = FMath::Max(MinFrameRate, GlobalRules.MaxFrameRate);
	...
	auto ValidateCandidate = [&](const FVdjmEncoderInitRequest& InitRequest, const EVdjmRecordQualityTiers CandidateTier)->bool
	{
		...
		const int32 DynamicBitrateBudget = FMath::Max(1000000, static_cast<int32>(ViewportPixels * 4));
		if (SafeVideoBitrate > (DynamicBitrateBudget * 2))
		{
			UE_LOG(LogVdjmRecorderCore, Warning,
				TEXT("ResolveEnvPlatform - Tier(%d) video bitrate too high. Bitrate=%d Budget=%d"),
				static_cast<int32>(CandidateTier),
				SafeVideoBitrate,
				DynamicBitrateBudget);
			return false;
		}
