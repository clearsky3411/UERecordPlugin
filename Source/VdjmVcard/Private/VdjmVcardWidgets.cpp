...
bool UVcardBackgroundBottomWidget::ApplyBottomSheetDescriptor(const FVcardBottomSheetDescriptor& bottomSheetDescriptor, FVcardDescriptorApplyResult& outResult)
{
	...
	for (const FVcardWidgetAttachDescriptor& headerAttachment : bottomSheetDescriptor.HeaderAttachments)
	{
		UUserWidget* createdWidget = nullptr;
		FString errorReason;
		const bool bApplied = UVcardDescriptorApplier::ApplyWidgetAttachment(request, headerAttachment, createdWidget, errorReason);
		if (bApplied)
		{
			bAnySuccess = true;
	...
	if (*bottomSheetDescriptor.ContentAttachment.WidgetClass)
	{
		FVcardWidgetAttachDescriptor contentAttachment = bottomSheetDescriptor.ContentAttachment;
		if (contentAttachment.TargetSlotName.IsNone())
		{
			contentAttachment.TargetSlotName = ContentSlotName;
		}

		UUserWidget* createdWidget = nullptr;
		FString errorReason;
		const bool bApplied = UVcardDescriptorApplier::ApplyWidgetAttachment(request, contentAttachment, createdWidget, errorReason);
		if (!bApplied)
		{
			outResult.ErrorReason = errorReason;
			return false;
	...
	SetOpenRatio(bottomSheetDescriptor.InitialOpenRatio);
	outResult.bSuccess = bAnySuccess;
	BP_OnBottomSheetDescriptorApplied(bottomSheetDescriptor);
	return bAnySuccess;
}

void UVcardBackgroundBottomWidget::SetOpenRatio(float openRatio)
{
	...
	mOpenRatio = FMath::Clamp(openRatio, minRatio, maxRatio);
	if (!FMath::IsNearlyEqual(previousRatio, mOpenRatio))
	{
