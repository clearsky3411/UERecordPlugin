...
## 가장 중요한 작성 규칙
- `WidgetClass`, `ObjectClass`, `ActorClass`, `SignalTag`, `SubgraphTag`처럼 동작 대상이 되는 값은 `None`이면 대부분 실패하거나 아무 일도 하지 않는다.
- `RuntimeSlotKey`는 현재 flow session 안에서만 쓰는 임시 이름이다.
...
## Key 개념 요약
| 이름 | 의미 | 생존 범위 | 주 용도 | None일 때 |
|---|---|---:|---|---|
| `EventTag` | event node 자체의 작성/디버그 태그 | DataAsset 작성 정보 | summary/debug에서 식별 | 동작에는 보통 영향 없음 |
| `RuntimeSlotKey` | flow session 내부 임시 객체 이름 | 현재 session | 다음 event가 객체/위젯을 다시 찾음 | 저장/조회하지 않음 |
| `ContextKey` | world context subsystem 전역 조회 이름 | World | 다른 session, widget, actor가 약참조로 찾음 | generic 등록 노드는 실패, 일부 ensure 노드는 표준 key 자동 사용 |
| `SignalTag` | flow signal 이름 | EventManager | wait, emit, branch trigger | wait/emit/branch는 보통 실패 또는 무효 |
| `EmitSignalTag` | event 성공 직후 자동 발행할 signal | EventManager | 생성 완료 알림 | 발행하지 않음 |
| `DestroySignalTag` | 위젯 자동 제거를 트리거할 signal | EventManager | intro/loading 종료 | `RemoveOnSignal`이면 필요 |
| `BranchTag` | subgraph branch rule 식별자 | EventManager | branch 교체/삭제/중복 판단 | `SignalTag`를 key처럼 사용 |
| `SubgraphTag` | DataAsset 안의 named subgraph 이름 | DataAsset | 시작할 subgraph 선택 | subgraph 실행 불가 |
| `FlowAsset` | subgraph를 찾을 FlowDataAsset | Asset | 다른 asset의 subgraph 실행 | 현재 main flow asset 사용 |
